...
#apply CART model
library(ipred)
fit_cart <- bagging(X1 ~., data=train)
...
# Create a rank variable based on importance
rankImportance_fit_cart = varImportance_fit_cart %>%
mutate(Rank = paste0('#',dense_rank(desc(importance_fit_cart))))


write(capture.output(rankImportance_fit_cart), "rankImportance_fit_cart model.csv")


# Predict using the test set

test_pred_cart = predict(fit_cart, test[,-1], type="class")
table(test_pred_cart, test[,1])

# summarize accuracy
xtab_cart = table(observed = test[,1], predicted = test_pred_cart)
cor(test_pred_cart,test$X1)
